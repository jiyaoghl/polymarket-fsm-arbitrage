...
## 19. 幻象失败防御与免签名 Data API 终极对账 (Phantom-Fill & Data API Fallback)
- **异常 OrderID 深度捕获**：
  - 跨国 VPS 网络高延迟环境下，下单接口遇到 HTTP 400（如 FOK killed）或 401（鉴权跨秒抖动）时，**严禁直接返回 None 导致状态机误判 FAILED**。
  - 必须从响应体中提取撮合层返回的 `orderID`，返回 `UNCONFIRMED` 态。
- **免签名 Data API 终极防线**：
  - 在 CLOB REST 轮询超时（>15s）或异常退出前，必须调用公共免签 Data API (`/trades?user=`) 对账链上真实成交。只要检测到真实买入，立即判定成功并推进至二腿对冲，坚决杜绝“链上已成交但代码误判失败”造成的孤儿单边裸奔。
  > **教训来源**：2026-08-22 实盘因 401 鉴权抖动导致代码判定首腿失败退出，但底层撮合引擎实际已成交该订单，最终产生 3 笔单边孤儿仓并在到期后全额归零。

## 20. 双腿并发限价做市与强平止损成交闭环 (Dual-GTC Bracket & Stop-Loss Closing)
- **Maker-Maker 专享并发双挂 (Dual Bracket)**：
