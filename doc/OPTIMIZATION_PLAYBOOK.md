# Polymarket 项目持续优化手册 (Optimization Playbook V4.0)
...
> **最后更新**：2026-09-04 V4.0 (高频WS推流器增量确认与防抖解耦落地彻底消灭生产震荡、微观OBI非对称倾斜做市、链上POL原生Gas水位防线、已赎回市场持久化预热、MQS深度强约束弹性资金落地、251 项全量单测绿灯、生产节点平滑热更验证完毕)
...
### 1.1 系统定位
针对 Polymarket CLOB V2 的 **5 分钟 Up/Down** 高频预测市场，执行 **Maker-Maker 零手续费做市 (主力) + 动态自适应防抖 Smart Re-peg + 四阶梯 OCO 做 T 变现 + 分批成交 1:1 精准对冲 + Garman-Klass+EWMA 极值波动率守护** 的高频全自动交易系统。
...
| **资金与风控** | `RiskManager` 双资金池 + 单市场排他锁 + TTL 强平 + **GK+EWMA 波动率防爆盾** | 真实日内极值方差提升 7.4x，平滑衰减历史污染，秒级释放未使用额度 |
...
### 1.3 核心基线指标 (V3.6 最新)
- **自动化测试用例**：**230 项单元测试 100% 绿灯覆盖**（含回测沙盒多资产 GK 守门与双模态归一化专项测试）；
- **微观分发延迟**：`P50=0.01ms`（已采样 1,846,000+ 帧盘口，零阻塞）；
...
5. **🎖️ 工程健康度**：全量 230+ 项单测绿灯、零内存泄漏、零事件循环阻塞。
...
- [x] **Garman-Klass 真实极值波动率估计器 + EWMA 衰减平滑** (V3.6 落地)
  - 融合 OHLC 四价方差估计，效率比收盘价 3σ 法提升 **7.4 倍**，消除平稳趋势均值漂移误杀，释放 **20%~30%** 良性做市机会
  - 施加负值 `clamp` 零点保护与 EWMA 半衰期 $T_{1/2}=4$ 指数衰减加权，平滑遗忘历史大跌
  - `net_change` 同步联动近 3 根真实 GK 均方根，接口 100% 零修改向下兼容
  - 专项测试 `test_kline_gk_ewma.py`（13 项全过）
- [x] **回测标定沙盒全面升级 Garman-Klass + EWMA 极值波动率** (V3.6 落地)
  - 快照录包服务（`l2_recorder.py`）增加资产归属显式持久化与 GK/3σ 双轨指标保存
  - 回测沙盒（`calibrate_params.py`）废除单一 BTC 写死，动态支持 BTC/ETH/SOL 币种自适应
  - 实现新旧快照双模态折算归一化（0.35x 系数），确保历史 50 万帧快照同构兼容
  - Optuna 采样空间校准至真实极值区间 `[0.02, 0.35]`，动态 OBI 守门乘数加固为 3.0x
  - 专项测试 `test_calibrator.py`（5 项全过）
- [x] **全量回归 230 项 100% 绿灯通过**（新增 2 项回测 GK 与双模态专项测试）
- [x] **地理合规确认**：VPS 新加坡节点通过 CLOB Geo-block 合规检验

### 阶段 6：推流防震荡解耦、做市微观倾斜与全链路加固 ✅ V4.0 全量落地
- [x] **高频 WS 推流器增量确认与防抖解耦 (Streamer Anti-Oscillation)**：
  - 维护 `confirmed_assets` 与 `pending_assets` 资产级双集合，已有资产常规行情绝不重置退避计数器；
  - 指数退避延展至 15.0s，强制施加 $\ge 2.0\text{s}$ 最小发送间隔严格节流阀，生产日志 `INVALID OPERATION` 彻底平息。
  - 专项测试 `test_streamer_anti_oscillation.py`（4 项全过）。
- [x] **微观非对称做市报价 (OBI Skew)**：
  - 采纳保守护盾型：$|OBI| > 0.25$ 时仅调低劣势侧挂单 $0.002 \sim 0.004$ 防毒流穿透，优势侧贴盘不追高。
  - 专项测试 `test_pricing_obi_skew.py`（4 项全过）。
- [x] **链上 Native POL 原生 Gas 资产实时水位防线**：
  - 查询钱包 POL 原生代币余额，POL < 0.5 时输出黄色预警，POL < 0.1 时触发实盘保护性熔断。
  - 专项测试 `test_risk_gas_watermark.py`（2 项全过）。
- [x] **已赎回市场 SQLite 持久化与启动预热**：
  - 新增 `redeemed_markets` 表，启动预热加载已赎回集合，彻底杜绝重启冗余 RPC 轮询。
  - 专项测试 `test_redeem_persistence.py`（2 项全过）。
- [x] **市场质量动态评分器 (MQS) 与深度硬约束弹性头寸**：
  - 融合价差、深度、波动率、OBI 综合打分（0~100 分），前 5 档深度 $\ge 80.0$ 份方可放大 1.5x 头寸。
  - 专项测试 `test_market_quality_score.py`（4 项全过）。
- [x] **单测作用域收敛**：`pyproject.toml` 显式声明 `testpaths = ["tests"]`，原生 `pytest` 100% 绿灯。
- [x] **全量自动化测试 251 项 100% 绿灯通过**。

### 阶段 7：全新纯净实盘钱包生成与真金小额闭环 🔮 下一阶段目标
- [ ] **全新纯净实盘钱包生成与安全上岗**：
...
4. **全量测试通过方可发布**：每次代码修改后必须运行 `pytest -s tests/` 确保 **230+ 项测试 100% 绿灯**；
