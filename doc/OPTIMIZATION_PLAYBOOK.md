...
### 阶段 1：止血与观测升级 (当前重点 · 1～2 天)
- [ ] **四阶梯做 T 降价脱手**：修改 `pending_leg2_handler.py`，实现 0~20s/20~45s/45~70s/70~85s 的阶梯式让价高抛逻辑，化解强平亏损；
- [ ] **Dashboard 订单生命周期透视抽屉 (Trade Inspector)**：在前端增加可视化时间轴，支持点击历史订单查看完整流转与追单轨迹；
- [ ] **策略级连续亏损熔断保护**：在 `RiskManager` 中增加单策略连续强平熔断机制；
- [ ] **全量单测覆盖**：新增单测覆盖阶梯做 T 与熔断保护。

### 阶段 2：策略加权与动态守门 (3～5 天)
- [ ] **资金向 Maker-Maker 大幅倾斜**：将 `maker_maker_conservative` 单笔金额由 3U 提升至 10U，评估重启标准做市策略；
- [ ] **动态 OBI 与深度壁垒函数**：将入场深度要求与 10m K 线振幅及剩余时间挂钩；
- [ ] **盘口成熟度综合打分**：引入买卖深度比、价差宽度与最近成交密度的综合评分守门。

### 阶段 3：真实 L2 盘口录包与离线参数优化 (1～2 周)
- [ ] **VPS 真实 L2 快照录包守护进程**：常驻后台采集真实 5min 盘口快照；
- [ ] **离线参数网格搜索与贝叶斯优化器 (`scripts/calibrate_params.py`)**：基于真实快照寻找最优参数组合。
