...
### 1.3 核心基线指标 (V3.6 最新)
- **自动化测试用例**：**228 项单元测试 100% 绿灯覆盖**（含新增 13 项 Garman-Klass 与 EWMA 衰减专项测试）；
...
5. **🎖️ 工程健康度**：全量 228+ 项单测绿灯、零内存泄漏、零事件循环阻塞。
...
- [x] **全量回归 228 项 100% 绿灯通过**（新增 13 项 GK+EWMA 专项测试）
...
4. **全量测试通过方可发布**：每次代码修改后必须运行 `pytest -s tests/` 确保 **228+ 项测试 100% 绿灯**；
